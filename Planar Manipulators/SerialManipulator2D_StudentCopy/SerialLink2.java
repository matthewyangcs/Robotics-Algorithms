...
//
    ...
    //joint angles is in radians
    //q[0], q[1], q[2] are the joint angles
    
    //calculate error in joint angles and remember to make angles within PI and -PI
    double[] error = new double[q.length];
    for (int i = 0; i < error.length; i++) {
      error[i] = qd[i] - q[i];
      //making angles between pi and - pi
      //shout out to piazza for the help
    ...
    //update new q
    for (int i = 0; i < res.length; i++) {
      res[i] = q[i] + Kp * error[i];
      //making angles between pi and - pi
      res[i] = (res[i] + Math.PI) % (2*Math.PI) - Math.PI;
    }
...
//
    ...
    double[][] inverseJacob = SerialLink2.inverseJacobian3(q);

    // 2.Calculate position error
    double[] error = new double[q.length];
    error[0] = xd - tool.x();
    error[1] = yd - tool.y();
    //we want the subtraction that yields the smallest angle between to different orientations
    error[2] = theta_d - tool.theta();
    if (Math.abs(tool.theta() - theta_d) < Math.abs(error[2])) {
      error[2] = tool.theta() - theta_d;
    }
    
    //standardize angle, but this shouldn't do anything.
    error[2] = (error[2] + Math.PI) % (2*Math.PI) - Math.PI; 
    
    //perform matrix multiplication
    //temp represents inverseJacob * Error
    double[] temp = new double[q.length];
    for (int row = 0; row < q.length; row++) {
      for (int col = 0; col < inverseJacob[0].length; col++) {
        temp[row] += inverseJacob[row][col] * error[col];
      }
    }

    // 3. Do Proportional Control on position. Use equation from slide 21 of https://dellaert.github.io/20S-3630/Slides/L11_Jacobian_Control.pdf
    //also use as reference https://dellaert.github.io/20S-3630/notes/3-manipulators.pdf
