...
Original file is located at
    https://colab.research.google.com/drive/13hStdxGjVjEnYsAenPbXaOLzkHDZbadD
...
Student #2 Name: Narae Lee

Student #3 Name: Matthew Yang

Student #4 Name: Ziyang (John) Zhang
...
class LaneFinder:
  ...
  def compute_orientation(self, image):
    ...
    #########################################################################
    for Z in self.scan_lines:
      Z_in_camera_frame_list.append(Z*math.cos(self.camera_pitch))

    # Z_in_camera_frame_list = self.scan_lines * math.cos(camera_pitch)
  ...
  def compute_v_coordinate_of_scan_line_in_image(self, Z):
    ...
    # ground.
    # use self.camera_pitch and self.height
    # see step 2 ("math behind steps")
    Y = self.height - math.sin(self.camera_pitch) * Z
    # Calculate v using the given Y
    v = self.v0 + self.f*Y/Z # l is 1?
    #########################################################################
    #                             END OF YOUR CODE                          
    #########################################################################
    return v
  ...
  def find_lane_boundaries_on_scan_line(self, image, v):
    ...
    """
    # need to be the middle of the lane
    u_left = 0
    u_right = 0
    ...
    # print("\n\n")
    # apply simple filter
    intensities = np.convolve(intensities, [.1, .8, .1], mode='same')
    
    # print(intensities.shape) (150,)
    print("after")
    print(intensities)
    # print("\n\n")
    ...
    # 200 will be our boundary for our cutoff for a white pixel
    left_lane_pixels = []
    right_lane_pixels = []

    # do the left
    for i in range((int)(len(intensities) / 2)):
      if intensities[i] >= 200:
        left_lane_pixels.append(i)

    # do the right
    for i in range((int)(len(intensities) / 2), len(intensities)):
      if intensities[i] >= 200:
        right_lane_pixels.append(i)

    # get the median for both arrays (already sorted)
    # print(left_lane_pixels)
    # print(right_lane_pixels)

    u_left = left_lane_pixels[int(len(left_lane_pixels) / 2)]
    u_right = right_lane_pixels[int(len(right_lane_pixels) / 2)]
  ...
  def compute_camera_coordinates_from_image_coordinates(self, u, v, Z):
    ...
    Y = self.height - math.sin(self.camera_pitch) * Z
    # this uses the formula above assuming k and l are 1
...
#########################################################################
image_path = '/content/drive/My Drive/CS 3630 project 5/ta_images/'
#########################################################################
#                             END OF YOUR CODE                          
#########################################################################
...
Z_cm = [50,100] 
camera_pitch_deg = 15
focal_length_in_pixels = 311.360353402 
height_of_camera_cm = 9.60
v0 = 240 
u0 = 320 
DEBUG = False
