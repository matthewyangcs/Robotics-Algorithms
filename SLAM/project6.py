...
Original file is located at
    https://colab.research.google.com/drive/1fxx-pvsXJNbHwoy5MgQmxj8BKGzV-5dD
...
def transform_cloud(bTa, clouda):
    ...
    # print(clouda)
    n = clouda.shape[1]
    # append row of 1's at end to be homogenous
    
    # Necessary
    ones = (np.ones(n)).reshape(1,n)
    # print("ones shape: " ,ones.shape)
    # print("clouda shape: ", clouda.shape)

    clouda = np.append(clouda, ones, axis = 0)
    # print(ones)
    # print(clouda)
    # print('test')
    ...
    matrix = bTa.matrix()
    newCloud = bTa.matrix().dot(clouda)
    # remove last row
    # print (newCloud)
    newCloud = newCloud[:3] # includes indices 0, 1, 2
    # print(newCloud)
    return newCloud
...
def assign_closest_pairs(clouda, cloudb):
    ...
    """
    # print("clouda  ",clouda.shape)
    baseCloud = np.transpose(clouda)
    neighbors = np.transpose(cloudb)

    n = baseCloud.shape[0]
    dim = baseCloud.shape[1]

    nbrs = NearestNeighbors(n_neighbors = 1, algorithm = 'kd_tree').fit(neighbors)
    # indices = nbrs.kneighbors(baseCloud, return_distance = False)
    distances, indices = nbrs.kneighbors(baseCloud)
    # prints sum of distances (make sure it's decreasing)
    # print(sum(distances))
    indices = np.reshape(indices, (n))

    # print("indices before resize: \n", indices,"\n")
    # print("cloud a shape", clouda.shape)
    # print(clouda)
...
def icp(clouda, cloudb, initial_transform=gtsam.Pose3(), max_iterations=25):
    ...
    for i in range(max_iterations):
      ...
      # transform points
      transformed_cloud = transform_cloud(bTa, clouda)

      # Assign each point in {Set 1} to its neighbor in {Set 2}
      nearestNeighbors = assign_closest_pairs(transformed_cloud, cloudb)

      # Add new points to icp_series
      icp_series.append([transformed_cloud, nearestNeighbors])
      
      # estimate transformation
      transform = estimate_transform(transformed_cloud, nearestNeighbors)

      # Adjust bTa
      # bTa = bTa.compose(transform)
      #
      bTa = bTa.compose(transform)
...
def populate_factor_graph(graph, initial_estimates, initial_pose, clouds):
    ...
    for i in range(len(clouds) - 1):
        ...
        cloudA = clouds[i]
        cloudB = clouds[i+1]
        T = icp(cloudA, cloudB, prev_T, max_iterations = 8)[0]
        prev_T = T
